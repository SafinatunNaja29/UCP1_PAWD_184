...
app.get('/', (req, res) => {
    mysql.query('SELECT * FROM data', (err, results) => {
        if (err) throw err;
...
app.post('/add', upload.single('foto'), (req, res) => {
    const { Nama, Pelayanan, Kecamatan } = req.body;
    const foto = req.file ? '/uploads/' + req.file.filename : null;

    mysql.query('INSERT INTO data (Nama, Pelayanan, Kecamatan ) VALUES (?, ?, ?)', [Nama, Pelayanan, Kecamatan ], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
...
app.get('/edit/:id', (req, res) => {
    ...
    mysql.query('SELECT * FROM data WHERE id = ?', [id], (err, results) => {
        if (err) throw err;
...
app.post('/edit/:id', upload.single('foto'), (req, res) => {
    ...
    mysql.query('UPDATE data SET Nama = ?, Pelayanan = ?, Kecamatan = ? WHERE id = ?', [Nama, Pelayanan, Kecamatan, id], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
...
app.get('/delete/:id', (req, res) => {
    const id = req.params.id;
    mysql.query('DELETE FROM data WHERE id = ?', [id], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
